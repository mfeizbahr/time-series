...
mydata <- read.csv('San Marcos.csv')
...
# Create an xts object for discharge data
WL.xts <- xts(mydata$Discharge, order.by = mydata$Date)

# Check for missing values
if (any(is.na(WL.xts))) {
  # Impute missing values using Kalman filter
  WL.xts <- na_kalman(WL.xts)
  print("Missing values imputed using Kalman filter.")
}
...
plot(MA30, xlab = 'Date', ylab = 'Discharge', main = '30-day Moving Average', col = "red")

# Plot the 90-day moving average
plot(MA90, xlab = 'Date', ylab = 'Discharge', main = '90-day Moving Average', col = "green")

# Plot the 365-day moving average
plot(MA365, xlab = 'Date', ylab = 'Discharge', main = '365-day Moving Average', col = "blue")
